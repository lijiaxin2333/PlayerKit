...
@MainActor
public final class PlayerPreNextPlugin: BasePlugin, PlayerPreNextService {
    ...
    public func startPreload() {
        guard let item = _nextItem, !_isPreloading else {
            print("[PlayerPreNextPlugin] 预加载失败: 无下一个视频或正在预加载")
            return
        }
        ...
        print("[PlayerPreNextPlugin] 开始预加载下一个视频: \(item.title ?? "无标题")")
        ...
        asset.loadValuesAsynchronously(forKeys: ["playable", "duration"]) { [weak self] in
            ...
            if playable {
                print("[PlayerPreNextPlugin] 预加载成功，时长: \(duration)秒")
                ...
                print("[PlayerPreNextPlugin] 预加载失败: 视频不可播放")
    ...
    public func cancelPreload() {
        ...
        print("[PlayerPreNextPlugin] 取消预加载")
    }
    ...
    private func simulatePreloadProgress() {
        ...
        Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { [weak self] timer in
            ...
            if self._preloadProgress >= 1.0 {
                ...
                print("[PlayerPreNextPlugin] 预加载完成")
            }
        }
    }
}
