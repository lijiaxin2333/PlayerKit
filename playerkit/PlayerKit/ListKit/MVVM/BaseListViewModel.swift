...
open class BaseListViewModel: ListBaseInfo {
    ...
    func updateListContext(_ context: ListContext?) {
    ...
    func createSectionViewModel(forData data: Any) -> BaseListSectionViewModel? {
    ...
    func notifyPlugins(_ action: (ListProtocol) -> Void) {
        for plugin in businessPlugins {
    ...
    func removeMsgObserver(_ observer: AnyObject, name: ListMsgName) {
        msgObservers[name]?.removeObject(forKey: observer)
    }

    func postMsg(_ name: ListMsgName, object: Any?, userInfo: [String: Any]?) {
