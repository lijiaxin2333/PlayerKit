...
@MainActor
public final class PlayerFullScreenComp: CCLBaseComp, PlayerFullScreenService {
    ...
    public func enterFullScreen(orientation: PlayerFullScreenOrientation = .auto, animated: Bool = true) {
        guard fullScreenState != .fullScreen,
            ...
            print("[PlayerFullScreenComp] 进入全屏失败: 播放器视图不可用")
            return
        }
        ...
        print("[PlayerFullScreenComp] 进入全屏, orientation: \(orientation)")
    ...
    public func exitFullScreen(animated: Bool = true) {
        ...
        print("[PlayerFullScreenComp] 退出全屏")
    ...
    private func createFullScreenWindow(with playerView: UIView, animated: Bool) {
        ...
        print("[PlayerFullScreenComp] 创建全屏窗口: \(window.bounds)")
    }
