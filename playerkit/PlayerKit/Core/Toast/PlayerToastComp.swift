...
@MainActor
public final class PlayerToastComp: CCLBaseComp, PlayerToastService {
    ...
    public func showToast(_ message: String, style: PlayerToastStyle = .info, duration: TimeInterval = 0) {
        ...
        print("[PlayerToastComp] 显示 Toast: \(message), 样式: \(style)")
        ...
        guard let playerView = engineService?.playerView else {
            print("[PlayerToastComp] 无法显示 Toast: 播放器视图不可用")
            return
        }
    ...
    public func showLoading(_ message: String? = nil) {
        print("[PlayerToastComp] 显示加载: \(message ?? "")")
        ...
        // 获取播放器视图作为父视图
        guard let playerView = engineService?.playerView else {
            print("[PlayerToastComp] 无法显示加载: 播放器视图不可用")
            return
        }
