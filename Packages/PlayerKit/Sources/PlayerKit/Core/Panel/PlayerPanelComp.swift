...
@MainActor
public final class PlayerPanelComp: CCLBaseComp, PlayerPanelService {
    ...
    public func showPanel(_ panel: AnyObject, at position: PlayerPanelPosition, animated: Bool) {
        guard let playerView = engineService?.playerView,
            ...
            print("[PlayerPanelComp] 无法显示面板: 播放器视图不可用或面板不是 UIView")
            return
        }
        ...
        print("[PlayerPanelComp] 显示面板: \(panel) at \(position)")
    }
    ...
    public func hideAllPanels(animated: Bool) {
        ...
        print("[PlayerPanelComp] 隐藏所有面板，数量: \(panels.count)")
    }
}
