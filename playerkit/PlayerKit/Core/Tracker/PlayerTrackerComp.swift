...
@MainActor
public final class PlayerTrackerComp: CCLBaseComp, PlayerTrackerService {
    ...
    public func registerTrackerNode(_ node: AnyObject) {
        guard let nodeProtocol = node as? PlayerTrackerNodeProtocol else {
            print("[PlayerTrackerComp] ⚠️ 节点未实现 PlayerTrackerNodeProtocol: \(node)")
            return
        }
        ...
        print("[PlayerTrackerComp] ✓ 注册节点: \(nodeName)")
    }
    ...
    public func unregisterTrackerNode(_ node: AnyObject) {
        ...
        print("[PlayerTrackerComp] ✗ 移除节点: \(nodeName)")
    }
