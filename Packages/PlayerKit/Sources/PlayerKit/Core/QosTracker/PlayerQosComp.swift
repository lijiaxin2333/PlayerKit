...
@MainActor
public final class PlayerQosComp: CCLBaseComp, PlayerQosService {
    ...
    public override func componentDidLoad(_ context: CCLContextProtocol) {
        ...
        self.context?.add(self, event: .playerPlayingStalledEnd) { [weak self] object, _ in
            ...
            print("[PlayerQosComp] ✓ 结束卡顿, 耗时: \(String(format: "%.2f", duration))秒")
        }
        ...
        self.context?.add(self, event: .playerReadyForDisplaySticky) { [weak self] _, _ in
            ...
            print("[PlayerQosComp] 🎬 首帧耗时: \(String(format: "%.2f", self._qosMetrics.startupTime))秒")
        }
    }
    ...
    public func startQosMonitoring() {
        ...
        print("[PlayerQosComp] 📊 开始 QoS 监控, 上报间隔: \(interval)秒")
    }
