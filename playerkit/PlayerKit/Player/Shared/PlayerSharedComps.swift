...
/// 当前播放器共享组件
@MainActor
public final class PlayerSharedCurrentPlayerComp: CCLSharedBaseComp, PlayerSharedCurrentPlayerService {
    ...
    private var _currentPlayer: CCLContextHolder?
    private var _currentPlayerType: PlayerType?

    // MARK: - PlayerSharedCurrentPlayerService

    public var currentPlayer: CCLContextHolder? {
    ...
    public required override init() {
        super.init()
        print("[PlayerSharedCurrentPlayerComp] 共享组件已创建")
    }

    // MARK: - PlayerSharedCurrentPlayerService

    public func setCurrentPlayer(_ player: CCLContextHolder?, type: PlayerType?) {
        _currentPlayer = player
        _currentPlayerType = type
        print("[PlayerSharedCurrentPlayerComp] 设置当前播放器: \(type?.rawValue ?? "unknown")")
    }

    public func clearCurrentPlayer() {
        _currentPlayer = nil
        _currentPlayerType = nil
        print("[PlayerSharedCurrentPlayerComp] 清除当前播放器")
    }
}
...
/// 全屏共享组件
@MainActor
public final class PlayerSharedFullScreenComp: CCLSharedBaseComp, PlayerSharedFullScreenService {
    ...
    public required override init() {
        super.init()
        print("[PlayerSharedFullScreenComp] 共享组件已创建")
    }
    ...
    public func enterFullScreen(with container: UIView?) {
        ...
        print("[PlayerSharedFullScreenComp] 进入全屏")
    }

    public func exitFullScreen() {
        _isFullScreen = false
        _fullScreenContainer = nil
        print("[PlayerSharedFullScreenComp] 退出全屏")
    }
...
/// 倍速共享组件（简化版）- 轻量级实现
@MainActor
public final class PlayerSharedSpeedCompSimple: CCLSharedBaseComp, PlayerSharedSpeedServiceSimple {
    ...
    public required override init() {
        super.init()
        print("[PlayerSharedSpeedComp] 共享组件已创建，默认倍速: 1.0x")
    }
    ...
    public func setSpeed(_ speed: Float) {
        guard _availableSpeeds.contains(speed) else {
            print("[PlayerSharedSpeedComp] 无效的倍速: \(speed)")
            return
        }
        ...
        print("[PlayerSharedSpeedComp] 设置倍速: \(speed)x")
    }
...
/// 循环播放共享组件
@MainActor
public final class PlayerSharedLoopingComp: CCLSharedBaseComp, PlayerSharedLoopingService {
    ...
    public required override init() {
        super.init()
        print("[PlayerSharedLoopingComp] 共享组件已创建")
    }
    ...
    public func setLooping(_ enabled: Bool) {
        _isLooping = enabled
        print("[PlayerSharedLoopingComp] 循环播放: \(enabled ? "启用" : "禁用")")
    }

    public func toggleLooping() {
        _isLooping.toggle()
        print("[PlayerSharedLoopingComp] 切换循环播放: \(_isLooping)")
    }
}
...
/// 定时关闭共享组件
@MainActor
public final class PlayerSharedTimedOffComp: CCLSharedBaseComp, PlayerSharedTimedOffService {
    ...
    public required override init() {
        super.init()
        print("[PlayerSharedTimedOffComp] 共享组件已创建")
    }
    ...
    public func setTimedOff(interval: TimeInterval) {
        ...
        if interval == 0 {
            ...
            startTime = nil
            print("[PlayerSharedTimedOffComp] 取消定时关闭")
            return
        }
        ...
        print("[PlayerSharedTimedOffComp] 设置定时关闭: \(interval / 60) 分钟")
    }
    ...
    private func handleTimedOff() {
        print("[PlayerSharedTimedOffComp] 定时关闭触发")
        timer = nil
        startTime = nil
        // 触发关闭事件（通过外部监听处理）
        // 注意：共享组件没有直接的事件发布能力
