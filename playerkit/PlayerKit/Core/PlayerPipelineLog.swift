...
public enum PLog {
    ...
    static func markCellVisible(_ index: Int) {
        cellVisibleTime[index] = CFAbsoluteTimeGetCurrent()
    }

    static func clearCellVisibleCache() {
    ...
    static func obtain(_ index: Int, source: String, poolCount: Int, playersAlive: [Int]) {
        write("OBTAIN", ["idx": index, "src": source, "pool": poolCount, "alive": playersAlive.sorted().description])
    }

    static func play(_ index: Int, needsSetURL: Bool, isReadyForDisplay: Bool, currentURL: String, playerViewFrame: String, playerViewInHierarchy: Bool) {
    ...
    static func evict(_ indices: [Int], poolCountAfter: Int) {
        guard !indices.isEmpty else { return }
        write("EVICT", ["idx": indices.sorted().description, "pool_after": poolCountAfter])
    }

    static func preRenderStart(_ id: String, activeCount: Int) {
        write("PR_START", ["id": id, "active": activeCount])
    }

    static func preRenderCancel(_ id: String, reason: String) {
        write("PR_CANCEL", ["id": id, "reason": reason])
    }

    static func preRenderReadyToPlay(_ id: String, elapsedMs: Int) {
        write("PR_READY_PLAY", ["id": id, "elapsed": elapsedMs])
    }

    static func preRenderReadyForDisplay(_ id: String, elapsedMs: Int) {
        write("PR_READY_DISPLAY", ["id": id, "elapsed": elapsedMs])
    }

    static func preRenderConsume(_ id: String, state: String) {
        write("PR_CONSUME", ["id": id, "state": state])
    }

    static func preRenderTake(_ id: String, state: String) {
        write("PR_TAKE", ["id": id, "state": state])
    }

    static func preRenderSkip(_ id: String, reason: String) {
        write("PR_SKIP", ["id": id, "reason": reason])
    }

    static func poolEnqueue(_ id: String, countAfter: Int) {
        write("POOL_ENQ", ["id": id, "count": countAfter])
    }

    static func poolDequeue(_ id: String, hit: Bool, countAfter: Int) {
        write("POOL_DEQ", ["id": id, "hit": hit, "count": countAfter])
    }

    static func engineSetURL(_ url: String, playerName: String) {
        write("ENGINE_SET_URL", ["url": String(url.suffix(40)), "player": playerName])
    }

    static func engineReadyToPlay(elapsedMs: Int, playerName: String) {
        write("ENGINE_READY_PLAY", ["elapsed": elapsedMs, "player": playerName])
    }

    static func engineReadyForDisplay(elapsedMs: Int, playerName: String) {
        write("ENGINE_READY_DISPLAY", ["elapsed": elapsedMs, "player": playerName])
    }

    static func scrollPrepare(_ index: Int) {
        write("SCROLL_PREPARE", ["idx": index])
    }

    static func scrollPlay(_ index: Int) {
        let c2p = cellToPlayMs(index)
        write("SCROLL_PLAY", ["idx": index, "cellToPlay": c2p])
    }

    static func coverFadeOut(_ index: Int) {
        let c2p = cellToPlayMs(index)
        write("COVER_FADEOUT", ["idx": index, "cellToPlay": c2p])
    }

    static func attachPlayer(_ index: Int, hasFirstFrame: Bool, coverHidden: Bool, playerViewFrame: String) {
        write("ATTACH", ["idx": index, "rfd": hasFirstFrame, "coverHidden": coverHidden, "pvFrame": playerViewFrame])
    }

    static func upgrade(_ playerName: String, hadEngine: Bool, rfdBefore: Bool, rfdAfter: Bool) {
        write("UPGRADE", ["player": playerName, "hadEngine": hadEngine, "rfdBefore": rfdBefore, "rfdAfter": rfdAfter])
    }

    static func playResume(_ index: Int, avPlayerRate: Float, avPlayerStatus: String) {
        write("PLAY_RESUME", ["idx": index, "rate": avPlayerRate, "status": avPlayerStatus])
    }

    static func cellCoverState(_ index: Int, coverAlpha: Double, coverHidden: Bool, coverHasImage: Bool, playerViewFrame: String) {
    ...
    static func renderViewState(_ index: Int, playerName: String, renderViewHidden: Bool, renderViewAlpha: Double, renderViewFrame: String, layerRFD: Bool, hasCurrentItem: Bool, avPlayerRate: Float) {
    ...
    static func viewHierarchy(_ index: Int, containerSubviewCount: Int, subviewDescriptions: String) {
        write("VIEW_HIERARCHY", ["idx": index, "subviewCount": containerSubviewCount, "subviews": subviewDescriptions])
    }

    static func prepareForReuse(_ index: Int) {
        write("CELL_REUSE", ["idx": index])
    }

    static func cellForItem(_ index: Int, hasPreRendered: Bool) {
        write("CELL_FOR_ITEM", ["idx": index, "hasPR": hasPreRendered])
    }

    static func willDisplayCell(_ index: Int, hasPlayer: Bool) {
        markCellVisible(index)
        write("WILL_DISPLAY", ["idx": index, "hasPlayer": hasPlayer])
    }

    static func detachPlayerLog(_ index: Int) {
        write("DETACH", ["idx": index])
    }

    static func preRenderPauseState(_ id: String, avRate: Float, layerRFD: Bool, timeControlStatus: String) {
        write("PR_PAUSE_STATE", ["id": id, "rate": avRate, "layerRFD": layerRFD, "timeCtrl": timeControlStatus])
    }

    static func firstFrameVisible(_ index: Int) {
        let c2p = cellToPlayMs(index)
        write("FIRST_FRAME_VISIBLE", ["idx": index, "cellToFirstFrame": c2p])
    }

    static func delayedPlayCheck(_ index: Int, rate: Float, timeCtrl: Int, rfd: Bool, currentTime: Double, rvFrame: String) {
        write("PLAY_CHECK_100ms", ["idx": index, "rate": rate, "timeCtrl": timeCtrl, "rfd": rfd, "t": String(format: "%.2f", currentTime), "rvFrame": rvFrame])
    }

    static func delayedPlayCheck300(_ index: Int, rate: Float, timeCtrl: Int, rfd: Bool, currentTime: Double) {
        write("PLAY_CHECK_300ms", ["idx": index, "rate": rate, "timeCtrl": timeCtrl, "rfd": rfd, "t": String(format: "%.2f", currentTime)])
    }

    static func bufferResume(_ playerName: String, source: String, buffered: Double, keepUp: Bool) {
        write("BUFFER_RESUME", ["player": playerName, "source": source, "buffered": String(format: "%.2f", buffered), "keepUp": keepUp])
    }

    static func bufferEmpty(_ playerName: String) {
        write("BUFFER_EMPTY", ["player": playerName])
    }

    static func itemFailed(_ playerName: String, error: String) {
        write("ITEM_FAILED", ["player": playerName, "error": error])
    }

    static func itemRetry(_ playerName: String) {
