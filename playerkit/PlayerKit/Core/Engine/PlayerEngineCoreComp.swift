...
@MainActor
public final class PlayerEngineCoreComp: CCLBaseComp, PlayerEngineCoreService {
    ...
    public override func componentDidLoad(_ context: CCLContextProtocol) {
        ...
        (self.context as? CCLContext)?.bindStickyEvent(.playerEngineDidCreateSticky, value: self)
    ...
    public func setURL(_ url: URL) {
        ...
        PLog.engineSetURL(url.absoluteString, playerName: (self.context as? CCLContext)?.name ?? "?")
        (self.context as? CCLContext)?.bindStickyEvent(.playerReadyForDisplaySticky, value: nil)
        (self.context as? CCLContext)?.bindStickyEvent(.playerReadyToPlaySticky, value: nil)
        createPlayerIfNeeded()
    ...
    private func createPlayer() {
        ...
        let readyForDisplayHandler: () -> Void = { [weak self] in
            ...
            PLog.engineReadyForDisplay(elapsedMs: elapsed, playerName: (self.context as? CCLContext)?.name ?? "?")
            (self.context as? CCLContext)?.bindStickyEvent(.playerReadyForDisplaySticky, value: self)
    ...
    private func configureAudioSession() {
        do {
            ...
            print("[PlayerEngineCoreComp] 音频会话配置失败: \(error)")
        }
    }
        ...
        playbackObserver = item.observe(\.status, options: [.new]) { [weak self] item, _ in
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                case .readyToPlay:
                    ...
                    let elapsed = Int((CFAbsoluteTimeGetCurrent() - self.setURLTime) * 1000)
                    PLog.engineReadyToPlay(elapsedMs: elapsed, playerName: (self.context as? CCLContext)?.name ?? "?")

                    if duration.isFinite {
                        self.context?.post(.playerDurationDidSet, object: duration, sender: self)
                    }

                    (self.context as? CCLContext)?.bindStickyEvent(.playerReadyToPlaySticky, value: self)
                    self.context?.post(.playerReadyToPlaySticky, object: self, sender: self)

                case .failed:
                    let pName = (self.context as? CCLContext)?.name ?? "?"
...
public class TimeObserverToken {
    let key: String
    nonisolated(unsafe) weak var engine: PlayerEngineCoreComp?

    init(key: String, engine: PlayerEngineCoreComp) {
        self.key = key
        self.engine = engine
    }
}
